...
import com.dicoding.toekangku1.data.dataworkerlocal.UserItem
...
import com.dicoding.toekangku1.response.main.GetWorkerResponse
...
class UserRepository private constructor(
    ...
    private val _workerRecommendations = MutableLiveData<GetWorkerResponse>()
    val workerRecommendations: LiveData<GetWorkerResponse> = _workerRecommendations
...
//                        _resetPasswordResponse.value = ResetPasswordResponse(null, error, message)
    ...
    fun getWorker( token: String){
        _isLoading.value = true
        val client = apiService.getWorker(token)

        client.enqueue(object: Callback<GetWorkerResponse> {
            override fun onResponse(
                call: Call<GetWorkerResponse>,
                response: Response<GetWorkerResponse>
            ) {
                try {
                    _isLoading.value=false
                    if (response.isSuccessful && response.body() != null){
                        _workerRecommendations.value = response.body()
            ...
            override fun onFailure(call: Call<GetWorkerResponse>, t: Throwable) {
                _isLoading.value=false
                when(t){
                    is UnknownHostException -> {
                        _toastText.value = Event("No Internet Connection")
    ...
    fun getUserbyType(type: String): UserItem{
        return userItem.first { it.item.type == type
        }
    }
